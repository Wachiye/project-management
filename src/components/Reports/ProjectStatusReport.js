...
const ProjectStatusReport = ({ project, projectSetting, thisYearSettings, user }) => {
  ...
  } = groupMilestones(project?.milestones);
  const { tasks, pendingTasks, completeTasks, tasksInProgress, taskProgressInPercentage } =
    groupTasks(getAllTasks(project?.milestones));
            ...
            <div className="card-body">
                ...
                <table className="table table-bordered w-100">
                  <tbody>
                    ...
                    <tr>
                      <th>Student</th>
                      <td>
                        <a href={`mailto:${project?.student?.user?.email}`}>
                    ...
                    <tr>
                      <th>Evaluator</th>
                      <td>
                        <a href={`mailto:${project?.evaluator?.user?.email}`}>
                          {project?.evaluator?.user?.fullName || "--"}
                        </a>
                      </td>
                    </tr>
                    <tr>
                      <th>Supervisor</th>
                      <td>
                        <a href={`mailto:${project?.supervisor?.user?.email}`}>
              ...
              <div className="card-text mb-2">
                ...
                {tasks.length > 0 ? (
                  ...
                  <p className="lead my-1 card-text  p-1">
                    Could not show Gantt Chart because project has no tasks
                  </p>
                )}
              </div>
