...
const CommentList = ({ comments }) => {
  return comments.map((comment) => <CommentCard comment={comment} key={comment?._id} />);
};
...
class Comments extends Component {
  ...
  setActive(active) {
    ...
    if (!active) this.getProjectAndComments();
  }

  async getProjectAndComments() {
    const response = await ProjectService.getOneById(this.state.projectId);
  ...
  render() {
    const { project, comments, alert, hasAlert, active } = this.state;
    return (
      <div className="admin-main">
        <div className="container">
          <div className="row">
            ...
            <div className="col-12 mb-2">{comments && <CommentList comments={comments} />}</div>
          </div>
        </div>
      </div>
    );
