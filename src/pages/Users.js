...
class Users extends Component {
    ...
    if (role === "STAFF") {
      ...
      if (AuthService.getUserRole() === "SUPERVISOR") {
        students = students?.filter((std) => {
          return std.projects?.filter((p) => p.supervisor?.user?.email === userEmail);
        });
      }
      this.setState({
        users: students
  ...
  async deleteUser(userId, role) {
    const del = window.confirm(`Are you sure you want to delete this ${role}?`);
    if (del) {
      isLoading(true);
      let response = null;
      if (role === "STUDENT") response = await StudentService.delete(userId);
      else if (["EVALUATOR", "SUPERVISOR", "STAFF"].includes(role))
        response = await StaffService.delete(userId);
      else response = await UserService.delete(userId);
  ...
  canDeleteUsers() {
    ...
    if (["ADMINISTRATOR", "EVALUATOR"].includes(role)) return true;
    else return false;
  }

  async deleteAllUsers(role) {
    const del = window.confirm(`Are you sure you want to delete all ${role} ?`);
    if (del) {
      isLoading(true);
      let response = null;
      if (role === "STUDENT") response = await StudentService.deleteAll();
      else response = await StaffService.deleteAll();
      if (response.error) {
        this.setAlert(response.error);
      } else if (role === "STUDENTS") await this.getAllStudents();
      else await this.getAllStaff();
      await this.getAllUsers();
      isLoading(false);
    }
  }
  ...
  render() {
    const { users, alert, hasAlert, userRole, canSelect } = this.state;
                  ...
                  {canSelect && (
                    <div className="card-text pull-left">
                      <div className="d-flex d-inline align-items-center">
                        <label htmlFor="userRole" style={{ width: "120px" }}>
                          User Role
                        </label>
                        <select
                          name="userRole"
                    ...
                    {this.canDeleteUsers() && (
                      <div className="btn-group">
                        ...
                        <button
                          ...
                          onClick={() => this.deleteAllUsers(userRole)}
                        >
