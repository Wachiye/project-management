...
class ViewGuide extends Component {
  ...
  async getGuide() {
    const guideId = this.state.guideId || this.props.match.params.guideId;
    const response = await GuideService.getOneById(guideId);
  ...
  render() {
    const { guide, hasAlert, alert } = this.state;
    return (
                ...
                <div className="card-header bg-transparent my-2">
                  <div className="owner shadow p-2">
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <span className="text-muted">
                        {DateFormat(guide?.createdAt).toDateString() || "--"}
                      </span>
                    </div>
                  </div>
                </div>
