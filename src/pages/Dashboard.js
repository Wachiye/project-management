...
class Dashboard extends Component {
    ...
    if (res.error) {
      ...
      const evaluator =
        res.data.data.filter(
          (s) => s.user?.email === AuthService.getUserEmail() && s.user?.role === "EVALUATOR"
        )[0] || null;
  ...
  render() {
    const { users, projects, students, staff, student } = this.state;
    return (
      <div className="admin-main">
        <div className="container-fluid p-1">
          <DashboardHeader />
          {AuthService.getUserRole() === "STUDENT" && (
            <StudentDashboard student={student} projects={projects} />
          )}
          ...
          {AuthService.getUserRole() === "SUPERVISOR" && (
            <SupervisorDashboard projects={projects} students={students} />
          )}
        </div>
      </div>
    );
