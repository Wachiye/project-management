...
class ViewMilestone extends Component {
  ...
  async getMilestone() {
    ...
    const response = await MilestoneService.getOneById(milestoneId);
  ...
  render() {
    const { milestone, hasAlert, alert } = this.state;
    return (
        ...
        <div className="container-fluid p-1">
          <div className="row">
            <div className="d-flex justify-content-between align-items-center">
              <Link
                to={`/projects/${milestone?.project?._id}/milestones`}
              ...
              <Link to={`/new-task/${milestone?._id}`} className="btn btn-sm btn-primary">
                New Task
              </Link>
            </div>
