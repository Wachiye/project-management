...
class Profile extends Component {
  ...
  async getProfileData() {
    ...
    if (userRole === "STUDENT") response = await StudentService.getAll();
    else response = await StaffService.getAll();

    if (response.error) {
      this.setAlert(response.error);
    } else {
      const profileData = response.data.data.filter(
        (user) => user.user?.email === AuthService.getUserEmail()
      )[0];
  ...
  async updateProfile() {
    isLoading(true);
    const { firstName, lastName, email, regNo, staffId, profileData, userRole } = this.state;
    const data = {
  ...
  async doUpdate(_id, role, data) {
    ...
    if (role === "STUDENT") response = await StudentService.update(_id, data);
    else response = await StaffService.update(_id, data);
  ...
  async changePassword() {
    isLoading(true);
    const { profileData, oldPassword, password, confirmPassword } = this.state;
    const data = {
      oldPassword,
      password,
  ...
  render() {
    const { userRole, profileData, alert, hasAlert } = this.state;
    return (
              ...
              <div className="card bg-transparent border-0 shadow" style={{ position: "relative" }}>
                ...
                <div className="card-body">
                  ...
                  <p className="card-text">
                    ...
                    <span className="mx-1">
                      {" "}
                      {`${userRole === "STUDENT" ? "Reg No" : "Staff ID"}`}:{" "}
                      {profileData?.regNo || profileData?.staffId}
                    </span>
                  </p>
                  <p className="card-text">
                  ...
                  <p className="card-text">
                    ...
                    <span className="mx-1">
                      Joined: {DateFormat(profileData?.user?.createdAt).toDateString()}
                    </span>
                  </p>
                  {userRole === "STUDENT" && (
                    <p className="card-text">
                      ...
                      <span className="pull-right">
                        <Link to="/my-projects" className="card-link">
                          View
                        </Link>
                      </span>
                    </p>
              ...
              <div className="card bg-transparent border-1">
                <div className="card-body">
                  ...
                  <RegistrationForm
                    ...
                    names={true}
                    email={true}
                    regNo={userRole === "STUDENT"}
                    staffId={userRole !== "STUDENT"}
                    disabled={false}
                    data={profileData}
                  />
                  <div className="my-2">
