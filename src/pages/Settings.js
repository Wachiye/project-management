...
class Settings extends Component {
  ...
  setActive(active) {
    ...
    if (!active) this.getAllSettingsByYear();
  }
  ...
  isSuperUser() {
    return (
      AuthService.getUserRole() === "EVALUATOR" || AuthService.getUserRole() === "ADMINISTRATOR"
    );
  }
  ...
  async deleteSetting(settingId) {
    const del = window.confirm("Are you sure you want to delete this setting");
    if (del) {
      isLoading(true);
      const response = await SettingService.delete(settingId);
      if (response.error) {
  ...
  render() {
    const { settings, alert, hasAlert, active } = this.state;
    return (
