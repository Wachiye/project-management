  ...
  if (student?.user?.email === AuthService.getUserEmail()) {
    return (
      <>
        ...
        <button className="btn btn-outline-secondary mx-1" onClick={() => setActive(true)}>
          Comment
        </button>
        <ModalContainer
          id="new-comment"
...
class MyProjects extends Component {
  ...
  async getStaff(id) {
    const response = await StaffService.getOneById(id);
  ...
  async changeStatus(project, status) {
    isLoading(true);
    const response = await ProjectService.setStatus(project?._id, status);
    if (response.error) {
      this.setAlert(response.error);
    } else {
      this.setState({
  ...
  async downloadFiles() {
    ...
    const projectFiles = currentProject?.projectFiles || [];
  ...
  async deleteProject() {
    isLoading(true);
    const response = await ProjectService.delete(this.state.projects[this.state.currentIndex]._id);
    if (response.error) {
      this.setAlert(response.error);
    } else {
      this.setState({
  ...
  render() {
    const { alert, hasAlert, active, projects, currentProject, user } = this.state;
    return (
          ...
          <div className="row">
            <div className="col-12 mb-2">
              <div className="d-flex justify-content-between align-items-center">
                ...
                <h3>My Projects</h3>
                {user?.role === "STUDENT" ? (
                  <Link to="/new-project" className="btn btn-sm btn-primary">
                    New Project
                  </Link>
                ) : (
                  <div> </div>
                ...
                <div className="card-body">
                  ...
                  <div className="pull-right mb-2">
                    ...
                    <Link
                      ...
                      to={`/projects/${currentProject?._id}/report`}
                    >
                      View Report
                    </Link>
                    {user?.role === "STUDENT" ? (
            ...
            <div className="col-12 my-2">
              <ul className="nav nav-tabs">
                <li className="nav-item">
                  <Link className="nav-link active" data-toggle="tab" to="#details">
                    Details
                  </Link>
                </li>
                <li className="nav-item">
                  <Link
                    className="nav-link"
                    data-toggle="tab"
                    to={`/projects/${currentProject?._id}/milestones`}
                  >
                    Milestones
                  </Link>
                </li>
                <li className="nav-item">
                  <Link
                    className="nav-link"
                    data-toggle="tab"
                    to={`/projects/${currentProject?._id}/files`}
                  >
                    Files
                    <span className="badge text-danger">
                      {currentProject?.projectFiles?.filter((f) => f.status === "PENDING")
                        ?.length || ""}
                    </span>
                  </Link>
                </li>
                <li className="nav-item">
                  <Link
                    className="nav-link"
                    data-toggle="tab"
                    to={`/projects/${currentProject?._id}/comments`}
                  >
                    Comments
                  </Link>
                </li>
                          ...
                          <ul className="list-group list-group-flush">
                            <li className="list-group-item">
                              ...
                              <span className="pull-right">
                                {DateFormat(currentProject?.startDate).toDateString()}
                              </span>
                            </li>
                            <li className="list-group-item">
                              <span className="mx-1">End Date</span>:{" "}
                              <span className="pull-right">
                                {DateFormat(currentProject?.endDate).toDateString()}
                              </span>
                            </li>
                            <li className="list-group-item">
                              <span className="mx-1">Status</span>:{" "}
                              <span className="pull-right">{currentProject?.status}</span>
                            </li>
                            <li className="list-group-item">
                              <span className="mx-1">Milestones</span>:{" "}
                              <span className="pull-right">
                                {currentProject?.milestones?.length || 0}
                              </span>
                            </li>
                            <li className="list-group-item">
                              <span className="mx-1">Time frame(days)</span>:{" "}
                              <span className="pull-right">{currentProject?.projectDays || 0}</span>
                            </li>
                            <li className="list-group-item">
                              <span className="mx-1">Time left(days)</span>:{" "}
                              <span className="pull-right">{currentProject?.daysLeft}</span>
                            </li>
                          </ul>
                        </div>
                      </div>
