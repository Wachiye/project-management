        ...
        {["WAITING_APPROVAL", "REJECTED"].includes(project?.status) && (
          <>
            <button
              ...
              onClick={() => changeStatus(project, "ACCEPTED")}
            >
              Accept
            </button>
            <button className="btn btn-danger" onClick={() => changeStatus(project, "REJECTED")}>
              Reject
            </button>
          </>
        )}
        ...
        {["ACCEPTED", "IN_PROGRESS"].includes(project?.status) && (
          <div className="input-group my-3">
            ...
            <select
              ...
              onChange={(evt) => setSupervisorId(evt.target.value)}
            >
              {supervisors &&
            ...
            <div className="input-group-append">
              <span
                ...
                onClick={() => setSupervisor(project, supervisorId)}
              >
                Assign
              </span>
            </div>
...
class ViewProject extends Component {
  ...
  async getSupervisors() {
    ...
    if (response.error) {
      ...
      this.setState({
        supervisors: response.data.data?.filter(
          (supervisor) => supervisor?.user?.role === "SUPERVISOR"
        )
      });
    }
  }
  ...
  async setSupervisor(project, supervisorId) {
    if (project?.evaluator?.user?.email === AuthService.getUserEmail()) {
      const response = await ProjectService.setSupervisor(project?._id, supervisorId);
      if (response.error) {
        this.setAlert(response.error);
      } else {
        this.setState({
  ...
  async getProjectsByCategory(category, id) {
    const res = await ProjectService.getAllByCategory(category);
    this.setState({
  ...
  async downloadFiles() {
    const { projectFiles } = this.state.project;
  ...
  render() {
    const { project, hasAlert, alert, supervisors, relatedProjects } = this.state;
    return (
              ...
              <div className="card bg-transparent border-0">
                <div className="card-header bg-transparent my-2">
                  <div className="owner shadow p-2">
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <span className="text-muted">
                        {DateFormat(project?.startDate).getFullYear() || "--"}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="card-body">
                  ...
                  <article className="card-text mb-1">
                    <h6 className="card-title text-success">Project Description</h6>
                    <TextToHTML text={project?.description} />
                  </article>
                  <div className="card-text mb-1">
                    <h6 className="card-title text-success">Programming languages</h6>
                    <p className="lead">{project?.languages}</p>
                  </div>
                  <div className="card-text mb-1">
                    <h6 className="card-title text-success">Project Category</h6>
                ...
                <div className="card-footer">
                  ...
                  <Link className="btn btn-secondary mx-1" to={`/projects/${project?._id}/report`}>
                    View Report
                  </Link>
                  <EvaluatorActions
                    project={project}
              ...
              <div className="well bg-light p-2">
                ...
                <ul className="list-unstyled px-2">
                  {relatedProjects.map((project) => (
                    <li key={project?._id}>
                      <Link to={`/projects/${project?._id}`} target={"_parent"}>
                        {project.name}
                      </Link>
                    </li>
                  ))}
